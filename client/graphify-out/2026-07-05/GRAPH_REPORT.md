...
## Corpus Check
- 23 files · ~6,296 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 128 nodes · 152 edges · 10 communities (9 shown, 1 thin omitted)
...
1. `expo` - 13 edges
2. `useConfigStore` - 13 edges
3. `useChatStore` - 8 edges
4. `scripts` - 6 edges
5. `adaptiveIcon` - 5 edges
6. `ios` - 4 edges
7. `android` - 4 edges
8. `parseContent()` - 4 edges
9. `extra` - 3 edges
10. `ChatScreen()` - 3 edges
...
- `SettingsScreen()` --calls--> `useConfigStore`  [EXTRACTED]
  app/settings.tsx → store/useConfigStore.ts
- `SetupScreen()` --calls--> `useConfigStore`  [EXTRACTED]
  app/setup.tsx → store/useConfigStore.ts
- `DrawerContent()` --calls--> `useChatStore`  [EXTRACTED]
  components/ui/DrawerContent.tsx → store/useChatStore.ts
...
Cohesion: 0.18
Nodes (11): RootLayout(), styles, SetupScreen(), styles, DrawerContent(), styles, ConfigState, secureConfigStorage (+3 more)
...
Cohesion: 0.18
Nodes (9): ChatScreen(), styles, SettingsScreen(), styles, ChatState, Message, Thread, useChatStore (+1 more)
...
## Knowledge Gaps
- **79 isolated node(s):** `mockReplace`, `mockSecureStore`, `formattedUrl`, `name`, `slug` (+74 more)
...
- **Why does `useConfigStore` connect `index.tsx` to `devDependencies`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **What connects `mockReplace`, `mockSecureStore`, `formattedUrl` to the rest of the system?**
